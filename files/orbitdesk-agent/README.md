...
|---|---|
| Hardware | CPU / RAM / GPU model |
| Embedding model load time | ___ s |
| Generation model load time | ___ s |
| Typical end-to-end latency per question | ___ s |
...
- **Known limitation:** the clarification question is currently templated
  (two hand-written variants keyed on whether "sync" appears in the
  question), not generated from retrieved context. With more time I'd have
  the LLM draft the clarification question, constrained to only ask about
  fields listed in `KB-008`'s "Information to Collect" section.
